...
def _path(value: Any, location: str) -> str:
    text = _string(value, location, maximum=4096)
    candidate = Path(text)
    ...
    if normalized != text:
        raise core_module.SchemaError(f"{location} must use normalized POSIX form")
    return normalized
...
def _validate_lifecycle_contract(operation: dict[str, Any]) -> dict[str, Any]:
    ...
    if operation_type == "git-stage":
        ...
        if expected["paths"] != paths:
            raise core_module.SchemaError(
                "expected_mutations.paths must exactly match inputs.paths"
            )
        ...
        if operation["guards"]["clean"] is not True:
            raise core_module.SchemaError("git-push requires a clean repository")
        ...
        for field in ("base", "head"):
            if not isinstance(inputs[field], str) or not BRANCH.fullmatch(
                ...
                raise core_module.SchemaError(f"inputs.{field} has invalid form")
...
def _ensure_exact_state(
    ...
    if branch != operation["guards"]["branch"]:
        raise core_module.GuardError("lifecycle branch changed before mutation")
    if head != operation["guards"]["head"]:
        raise core_module.GuardError("lifecycle HEAD changed before mutation")
...
def _execute_stage(
    ...
    if conflicts:
        raise core_module.GuardError("unmerged paths prevent governed staging")
    staged_before = _git(
        supervisor, root, commands, "diff", "--cached", "--name-only",
        phase="verify empty starting index",
    )
    if staged_before:
        raise core_module.GuardError("index must be empty before git-stage")
    _git(
...
def _execute_commit(
    ...
    if parent != expected["parent_head"]:
        raise core_module.ExecutorError("created commit has unexpected parent")
    committed = _git(
...
def _execute_push(
    ...
    if status:
        raise core_module.GuardError("git-push requires a clean index and worktree")
    branch = _git(
        supervisor, root, commands, "branch", "--show-current",
        phase="verify attached push branch",
    )
    if not branch:
        raise core_module.GuardError("git-push refuses detached HEAD")
    remote = expected["remote"]
    ...
    if remote_before:
        ancestor = supervisor.run(
            ["git", "merge-base", "--is-ancestor", remote_before, expected["head"]],
            root,
            timeout_seconds=30,
...
def _execute_pull_request(
    ...
    existing = _run(
        ...
        root,
        [
            "gh", "pr", "list", "--state", "open", "--head", inputs["head"],
            "--base", inputs["base"], "--json", "number,url,headRefOid",
        ],
        commands,
    ...
    command = [
        "gh", "pr", "create", "--base", inputs["base"], "--head", inputs["head"],
    ...
    view = _run(
        ...
        [
            ...
            "number,url,state,isDraft,baseRefName,headRefName,headRefOid,title,body",
        ],
        commands,
...
def _execute_lifecycle(
    ...
    try:
        ...
        progress.phase(3, f"performing {operation['operation_type']}")
        result["mutation"]["attempted"] = True
        if operation["operation_type"] == "git-stage":
            evidence = _execute_stage(root, supervisor, operation, commands)
            terminal = "local-mutation-completed"
        elif operation["operation_type"] == "git-commit":
            evidence = _execute_commit(root, supervisor, operation, commands)
            terminal = "commit-completed"
        elif operation["operation_type"] == "git-push":
            evidence = _execute_push(root, supervisor, operation, commands)
    ...
    except (core_module.ExecutorError, OSError, ValueError, json.JSONDecodeError) as exc:
...
def lifecycle_main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        progress.phase(1, "validating lifecycle operation description")
        operation = _load_lifecycle_operation(operation_path)
        ...
        print("", flush=True)
        result, destination = _execute_lifecycle(operation, progress)
        print("", flush=True)
        print("-" * 60, flush=True)
...
elif operation_type in LIFECYCLE_OPERATION_TYPES:
    main = lifecycle_main
else:
